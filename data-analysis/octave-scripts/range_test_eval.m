...
if plot_true == 1
  ...
  figure
  p1 = subplot(2,1,1);
  plot(range_interp, 70*rc_rssi-120,'.');
  ...
  % TM RSSI vs Range
  nearest_idx = arrayfun(@(t) find(abs(gps_dt - t) == min(abs(gps_dt - t)), 1), tm_dt);
  range_sparse = range(nearest_idx);

  figure
  p3 = subplot(2,1,1);
  plot(range_sparse, tm_rssi_local/255, '.');
  title('Telemetry Datalink Quality');
  hold on;
  plot(range_sparse, tm_rssi_remote/255, '.');
  xlabel('Range [km]');
  ylabel('RSSI [%]');
  legend({'MAVLink - GCS';'MAVLink - Remote'});
  grid on;

  p4 = subplot(2,1,2);
  plot(range_sparse, tm_noise_local/255, '.');
  hold on;
  plot(range_sparse, tm_noise_remote/255, '.');
  xlabel('Range [km]');
  ylabel('Noise Level [?]');
  legend({'MAVLink - GCS';'MAVLink - Remote'});
  grid on;
  ...
  figure
  p5 = plot(range_sparse, tm_rx_err);
  xlabel('Range [km]');
  ylabel('Lost Packets');
  grid on;
end
...
%y_small_interp = interp1(t_small, y_small, t_big, 'linear', 'extrap');
